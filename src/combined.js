...
import {FLIGHT_PRICE_NOTICE,resolveAirportCode,searchAirports,searchFlights,showResolvedAirport,validateFlightQuery} from './flights.js?v=2.2.0';
...
export function mountCombinedSearch(container,{providersPromise}={}){
  ...
  const updateTotal=()=>{
    ...
    const message=['✈️🏨 Alufivia · Hotel + vuelo',`${form.elements.origin.value} → ${form.elements.destination.value}`,`${form.elements.departureDate.value} → ${form.elements.returnDate.value}`,`Vuelo: ${money(flight.price,flight.currency)}`,`Hotel: ${hotel.name} · ${money(hotel.totalPrice,hotel.currency)}`,`Total estimado: ${money(total,flight.currency||hotel.currency)}`,flightUrl,hotel.url||''].filter(Boolean).join('\n');
