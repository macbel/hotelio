...
function itemCard(item,kind){
  ...
  return `<article class="account-item"><span class="account-item-icon">${icons[type]||'♡'}</span><div><strong>${esc(item.label)}</strong><small>${esc(labels[type]||'Búsqueda')} · ${kind==='alert'?`cada ${item.frequency_hours} h · caduca ${date(item.expires_at)}`:`guardada ${date(item.created_at)}`}</small></div><div class="account-item-actions">${open}<button class="danger" type="button" data-delete-kind="${kind}" data-delete-id="${item.id}">Eliminar</button></div></article>`;
}
