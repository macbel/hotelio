...
function itemCard(item,kind){
  const icons={hotel:'⌂',flight:'✈',combined:'✈+'},type=item.type||'hotel';
  return `<article class="account-item"><span class="account-item-icon">${icons[type]||'♡'}</span><div><strong>${esc(item.label)}</strong><small>${esc(type==='combined'?'Hotel + vuelo':type==='flight'?'Vuelo':'Hotel')} · ${kind==='alert'?`cada ${item.frequency_hours} h · caduca ${date(item.expires_at)}`:`guardada ${date(item.created_at)}`}</small></div><button class="danger" data-delete-kind="${kind}" data-delete-id="${item.id}">Eliminar</button></article>`;
}
...
async function saveLast(kind,frequencyHours){
  ...
  else await request('account.php',{method:'POST',body:{action:'create_alert',query:lastSearch.query,frequencyHours,...base}});
  await showAccount();
}
...
function showSaveBar(detail){lastSearch=detail;let bar=document.querySelector('#searchActions');if(!bar){bar=document.createElement('aside');bar.id='searchActions';bar.className='search-actions';document.body.append(bar)}bar.innerHTML=`<span><strong>${esc(detail.label||'Búsqueda lista')}</strong><small>Guárdala o vigila cualquier cambio de precio durante 7 días.</small></span><button data-save-search>Guardar búsqueda</button><label>Alerta <select data-alert-frequency><option value="12">cada 12 h</option><option value="24" selected>diaria</option><option value="72">cada 3 días</option><option value="168">semanal</option></select></label><button data-create-alert>Crear alerta</button><button class="account-close" data-close-actions>×</button>`;bar.querySelector('[data-save-search]').onclick=()=>saveLast('search');bar.querySelector('[data-create-alert]').onclick=()=>saveLast('alert',Number(bar.querySelector('[data-alert-frequency]').value)).catch(error=>alert(error.message));bar.querySelector('[data-close-actions]').onclick=()=>bar.remove()}
