...
if($action==='save_search'){
  $filters=$body['filters']??null;$type=(string)($body['type']??'hotel');$label=trim((string)($body['label']??'Búsqueda guardada'));if(!is_array($filters)||!in_array($type,array('hotel','flight','combined'),true))vuelotel_json(400,array('error'=>'Búsqueda no válida.'));
...
if($action==='create_alert'){
  $query=$body['query']??null;$frequency=(int)($body['frequencyHours']??24);$type=(string)($body['type']??'hotel');if(!is_array($query)||!in_array($frequency,array(12,24,72,168),true)||!in_array($type,array('hotel','flight','combined'),true))vuelotel_json(400,array('error'=>'Alerta no válida.'));
