...
if($action==='create_alert'){
  ...
  if($type==='destination'){$mode=(string)($body['alertMode']??'lower');$threshold=(float)($body['threshold']??0);if(!in_array($mode,array('lower','threshold'),true)||($mode==='threshold'&&$threshold<=0))vuelotel_json(400,array('error'=>'Configura una condición de alerta válida.'));$query['_alertMode']=$mode;$query['_threshold']=$threshold;}
